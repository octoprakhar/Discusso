...
import { getAllDrafts } from "@/app/_libs/data-service";
...
export default async function Page() {
  ...
  */
  ...
  try {
    ...
    drafts = await getAllDrafts(userId);
    console.log("Got drafts as, ", drafts);
  } catch (err) {
    console.error(
      "💣 Drafts page.js: Error occured while fetching drafts: ",
      err
    );
  }
  ...
  return (
    <div className="space-y-6 px-2 py-1">
      ...
      <div className="space-y-4">
        {drafts.map((draft) => (
          <DraftPost draft={draft} key={draft.draftId} />
        ))}
      </div>
    </div>
  );
